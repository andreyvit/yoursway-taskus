    ...
    public void run(DocumentWindow window, AProjectName projectName) {
        ...
        for (MElement element : structure.getChildren())
            if (element instanceof MProject) {
                ...
                if (projectName == null || projectName.equals(project.getLine().name())) {
    ...
    public static void collectSynchronizationDefinitions(MProject project,
        ...
        for (MElement element : project.getChildren())
            if (element instanceof MText) {
                ...
                if (source != null)
                    synchronizationDefinitions.add(new SynchronizationDefinition((MText) element, source));
            }
    }
    ...
    private void synchronize(MProject project, final SynchronizationDefinition definition, File animals) {
        ...
        if (nafNafWithRedTail.exists() && nifNifWithGreenTail.exists())
            try {
                ...
                synchronizer.setOldLocalTasks(parseTasks(readAsString(nafNafWithRedTail), idTagName));
